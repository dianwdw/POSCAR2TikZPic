...
#POSCAR2TikZPicV1.7.sh 20180325
#The positions of POSCAR file can be given in Cartesian or direct(fractional) coordiantes
...
#Define atom radius
ElementRadius[1]=0.2;ElementRadius[2]=0.2;ElementRadius[3]=0.2;ElementRadius[4]=0.2;ElementRadius[5]=0.2;ElementRadius[6]=0.2;
...
ElementCol[1]="black";ElementCol[2]="magenta";ElementCol[3]="blue";ElementCol[4]="red";ElementCol[5]="green";ElementCol[6]="yellow";
...
if [[ $FirstChar = "S" ]] || [[ $FirstChar = "s" ]];then
  NHeader=9
  Var1=$(sed -n '9p' ${InputCoords} | awk '{print $1}')
  FirstChar1=$(echo ${Var1:0:1})
  if [[ $FirstChar1 = "D" ]] || [[ $FirstChar1 = "d" ]];then
    CoordSystem="Direct"
    echo "The positions of ${InputCoords} file are given in direct(fractional) coordiantes"
  elif [[ $FirstChar1 = "C" ]] || [[ $FirstChar1 = "c" ]];then
    CoordSystem="Cartesian"
    echo "The positions of ${InputCoords} file are given in Cartesian coordiantes"
  fi
elif [[ $FirstChar = "D" ]] || [[ $FirstChar = "d" ]] || [[ $FirstChar = "C" ]] || [[ $FirstChar = "c" ]] ;then
  NHeader=8
  if [[ $FirstChar = "D" ]] || [[ $FirstChar = "d" ]];then
    CoordSystem="Direct"
    echo "The positions of ${InputCoords} file are given in direct(fractional) coordiantes"
  elif [[ $FirstChar = "C" ]] || [[ $FirstChar = "c" ]];then
    CoordSystem="Cartesian"
    echo "The positions of ${InputCoords} file are given in Cartesian coordiantes"
  fi
fi
...
do
	...
	z=$(sed -n ''$j'p' ${InputCoords} | awk '{print $3}')
        if [[ $CoordSystem = "Direct" ]];then
  	  car[1]=$(echo "scale=6;(${x})*(${L[1]})-(${RotCenter[1]})" | bc)
  	  car[2]=$(echo "scale=6;(${y})*(${L[2]})-(${RotCenter[2]})" | bc)
  	  car[3]=$(echo "scale=6;(${z})*(${L[3]})-(${RotCenter[3]})" | bc)
        elif [[ $CoordSystem = "Cartesian" ]];then
          car[1]=$(echo "scale=6;(${x})-(${RotCenter[1]})" | bc)
          car[2]=$(echo "scale=6;(${y})-(${RotCenter[2]})" | bc)
          car[3]=$(echo "scale=6;(${z})-(${RotCenter[3]})" | bc)
        fi
